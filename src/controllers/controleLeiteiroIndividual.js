...
const cadastrarProducaoIdividual = async (req, res) => {
  ...
  try {
    ...
    if (!vaca) {
      return res
        .status(404)
        .json({ mensagem: "O Id do animal informado não foi encontrada." })
    }
...
const atualizarProducaoIndividual = async (req, res) => {
  ...
  try {
    ...
    const producao = await consultarDadosUnicos(
      "controle_leiteiro_individual",
      { id, id_propriedade }
    )
    ...
    if (!producao) {
      return res
        .status(404)
        .json({ mensagem: "Cadastro de producão individual não encontrado." })
    }
...
const excluirProducaoindividual = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ mensagem: `Cadastro de producão individual excluido.` })
  } catch (error) {
    return res.status(500).json({ mensagem: "Erro interno do servidor." })
  }
}
