...
const cadastrarChuva = async (req, res) => {
  ...
  try {
    ...
    const chuvaCadastrada = await cadastrarDados(
      "controle_pluviometrico",
      chuva
    )
...
const AtualizarChuva = async (req, res) => {
  ...
  try {
    ...
    const chuva = await consultarDadosUnicos("controle_pluviometrico", {
      id,
      id_propriedade
    })
    ...
    if (!chuva) {
      return res
        .status(404)
        .json({ mensagem: "Cadastro de chuva não encontrado." })
    }

    const chuvaAtualizada = {
      id_produtor,
      id_propriedade,
      data_chuva,
      volume_de_chuva
